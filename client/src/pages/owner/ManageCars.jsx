...
const ManageCars = () => {
  ...
  const deleteCar = async(carId) => {
    try {
      const confrim = window.confirm('Are you sure you want to delete this car?')
            ...
            {cars.map((car, index) => (
              <tr key={index} className='border-t border-borderColor'>
                <td className='p-3 flex items-center gap-3'>
                  <img src={car.image} alt="" className='h-12 w-12 aspect-sqaure object-cover rounded-md' />
                ...
                <td className='p-3 max-md:hidden'>{currency}{car.category}/day</td>

                <td className='p-3 max-md:hidden'>
                  <span className={`px-3 py-1 rounded-full text-xs ${car.isAvaliable ? 'bg-green-400/15 text-green-600' : 'bg-red-400/15 text-red-600'}`}>
